...
const Workout = ({users}: Data) => {
  return (
    <section id="about" className=" text-white pt-20 max-sm:pt-10 bg-opacity-95 bg-black ">
        ...
        <div className="mx-auto w-[90%] max-lg:w-[90%] flex  font-dmsans md:flex ">
    <div className="flex justify-evenly max-md:grid max-md:grid-cols-3 max-sm:grid-cols-2 w-[100%]">
        {users.map((user) => (
            <div className="my-4 mx-2 relative">
                ...
                <button className=" absolute border justify-center flex py-2 w-[80%] px-3 max-sm:px-2 text-[13px] font-normal rounded-md border-white bottom-4 left-1/2 transform -translate-x-1/2  text-white z-10">
                    {user.button}
                </button>
            </div>
        ))}
